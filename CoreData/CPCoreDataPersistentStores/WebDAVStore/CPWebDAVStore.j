...
- (void) saveAll:(CPSet) objects error:({CPError}) error
{
	...
	{
		[[self davManager] writeFileWithStringContent:[objects serializeToXML:YES containsChangedProperties:NO] toPath:[self storeID]];
	}
	else if([[self format] isEqualToString:CPCoreDataSerialization280NPLISTFormat])
	{
		[[self davManager] writeFileWithStringContent:[[objects serializeTo280NPLIST:YES containsChangedProperties:NO] rawString] toPath:[self storeID]];
	}
	else if([[self format] isEqualToString:CPCoreDataSerializationJSONFormat])
	{
		[[self davManager] writeFileWithStringContent:[objects serializeToJSON:YES containsChangedProperties:NO] toPath:[self storeID]];		
	}	
